...
from agents.props_analysis_agent import PropsBreakdown
...
from agents.scene_analysis_agent import SceneBreakdown
from agents.timeline_analysis_agent import TimelineBreakdown
...
def create_default_analyses_complete() -> Dict[str, bool]:
    """Create default analyses complete dictionary"""
    return {analysis_type: False for analysis_type in ANALYSIS_TYPES}

def create_default_needs_revision() -> Dict[str, bool]:
    """Create default needs revision dictionary"""
    return {analysis_type: False for analysis_type in ANALYSIS_TYPES}
...
class ScriptAnalysisState(BaseModel):
    ...
    needs_revision: Annotated[Dict[str, bool], merge_dict] = Field(default_factory=create_default_needs_revision)
    
    # Completion tracking
    analyses_complete: Annotated[Dict[str, bool], merge_dict] = Field(default_factory=create_default_analyses_complete)
