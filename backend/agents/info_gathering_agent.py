...
# Constants
SCENE_TEXT_LIMIT = 2000
WORDS_PER_PAGE = 250
SCENE_HEADER_PATTERNS = [
    # Standard INT/EXT patterns with flexible spacing and punctuation
    r'^(INT\.?|EXT\.?)\s*\.?\s*[A-Z][A-Z\s\-]*\s*-\s*(DAY|NIGHT|DAWN|DUSK)',
    # Standalone numbers (scene numbers)
    r'^\s*\d+\s*$',
    # INT/EXT with location but no time
    r'^(INT\.?|EXT\.?)\s*\.?\s*[A-Z][A-Z\s\-]*',
    # BABAK format (Malaysian scripts)
    r'^BABAK\s+\d+:\s*(INT\.?|EXT\.?)',
    # Numbers followed by scene headers on same line
    r'^\s*\d+\s+(INT\.?|EXT\.?)',
]
...
async def extract_script_data(script_content: str, human_feedback: Dict[str, str] = None) -> RawScriptData:
    ...
    try:
        ...
        if human_feedback:
            print("📝 Incorporating human feedback into data extraction...")
            script_content = _incorporate_feedback_into_extraction(script_content, human_feedback)
        
        scenes_raw_data = _parse_scenes(script_content)

        # Validate scene count
        if not _validate_scene_count(scenes_raw_data, script_content):
            print("🔄 Retrying scene parsing with alternative method...")
            scenes_raw_data = _alternative_scene_parsing(script_content)
        ...
        for i, (scene_text, scene_num) in enumerate(scenes_raw_data):
            ...
            try:
                limited_scene = scene_text[:SCENE_TEXT_LIMIT] if len(scene_text) > SCENE_TEXT_LIMIT else scene_text
...
def _incorporate_feedback_into_extraction(script_content: str, feedback: Dict[str, str]) -> str:
    """Incorporate human feedback into script content processing"""
    feedback_context = []
    
    for analysis_type, feedback_text in feedback.items():
        if feedback_text.strip():
            feedback_context.append(f"{analysis_type.upper()} FEEDBACK: {feedback_text}")
    
    if feedback_context:
        # Add specific instructions for scene counting
        scene_instructions = """
        SCENE COUNTING INSTRUCTIONS:
        - Look for numbered scenes (1, 2, 3, etc.) followed by scene headers
        - Count each INT./EXT. location change as a new scene
        - Pay attention to scene numbers in the script
        - Verify the total count matches the highest scene number
        """
        feedback_header = "HUMAN FEEDBACK FOR RE-ANALYSIS:\n" + "\n".join(feedback_context) + "\n" + scene_instructions + "\nORIGINAL SCRIPT:\n"
        return feedback_header + script_content
...
def _parse_scenes(script_content: str) -> List[tuple]:
    ...
    while i < len(lines):
        ...
        # Check if current line is a scene number
        is_scene_number = re.match(r'^\s*\d+\s*$', line)
        
        # Check if current line is a scene header
        is_scene_header = any(re.match(pattern, line, re.IGNORECASE) 
                             for pattern in SCENE_HEADER_PATTERNS)
        
        # Check if scene number is followed by scene header
        is_numbered_scene = (is_scene_number and 
                           i < len(lines) - 1 and 
                           any(re.match(r'^(INT\.?|EXT\.?)', lines[i+1].strip(), re.IGNORECASE)))
        
        # Check for scene header on same line as number
        is_combined_scene = re.match(r'^\s*\d+\s+(INT\.?|EXT\.?)', line, re.IGNORECASE)
        
        if is_scene_header or is_numbered_scene or is_combined_scene:
            ...
            # If it's a numbered scene, include the next line (scene header)
            if is_numbered_scene and i < len(lines) - 1:
                i += 1
                current_scene.append(lines[i])
        else:
...
def _parse_scene_manual(scene_text: str, scene_num: int) -> SceneData:
    ...
    for line in lines:
        ...
        header_match = _match_scene_header(line)
        if header_match:
            scene_header = line
            scene_type, location_time = header_match
            location, time_of_day = _parse_location_time(location_time)
            break
    ...
    for line in lines[1:]:
        ...
        if _is_character_name(line):
            ...
            _extract_props_and_special(line, props_mentioned, special_requirements)
            _categorize_content(line, characters, dialogue_lines, action_lines)
    ...
    return SceneData(
        ...
        estimated_pages=max(0.1, len(scene_text.split()) / WORDS_PER_PAGE),
        props_mentioned=list(set(props_mentioned)),
        special_requirements=special_requirements
    )

def _validate_scene_count(scenes: List[tuple], script_content: str) -> bool:
    """Validate that scene count matches numbered scenes in script"""
    # Find all scene numbers in the script
    scene_numbers = re.findall(r'^\s*(\d+)\s*$', script_content, re.MULTILINE)
    
    if scene_numbers:
        expected_count = len(scene_numbers)
        actual_count = len(scenes)
        
        print(f"🔍 Scene validation: Expected {expected_count}, Found {actual_count}")
        
        if expected_count != actual_count:
            print(f"⚠️ Scene count mismatch! Expected: {expected_count}, Actual: {actual_count}")
            return False
    
    return True

def _alternative_scene_parsing(script_content: str) -> List[tuple]:
    """Alternative scene parsing method for difficult scripts"""
    lines = script_content.split('\n')
    scenes = []
    current_scene = []
    
    # Look for any line that starts with a number
    for i, line in enumerate(lines):
        line_stripped = line.strip()
        
        # More aggressive scene detection
        if (re.match(r'^\s*\d+\s*$', line_stripped) or  # Standalone numbers
            re.match(r'^\s*\d+\s*INT', line_stripped, re.IGNORECASE) or  # Number + INT
            re.match(r'^\s*\d+\s*EXT', line_stripped, re.IGNORECASE)):   # Number + EXT
...
def _match_scene_header(line: str) -> tuple:
    """Match scene header and extract type and location"""
    ...
    if match:
        ...
        return scene_type, location_time
    return None

def _parse_location_time(location_time: str) -> tuple:
    """Parse location and time from location_time string"""
    for separator in [' – ', ' - ']:
        if separator in location_time:
            location, time_of_day = location_time.split(separator, 1)
            return location.strip(), time_of_day.strip().upper()
    return location_time, "DAY"

def _is_character_name(line: str) -> bool:
    """Check if line is a character name"""
    return (line.isupper() and len(line.split()) <= 4 and len(line) > 1 and
            not line.startswith(EXCLUDED_PREFIXES))

def _extract_props_and_special(line: str, props_mentioned: List[str], special_requirements: List[str]):
    """Extract props and special requirements from line"""
    line_lower = line.lower()
    
    for prop in PROP_KEYWORDS:
        if prop.lower() in line_lower:
            props_mentioned.append(prop)
    
    if any(special in line_lower for special in SPECIAL_KEYWORDS):
        special_requirements.append(line)

def _categorize_content(line: str, characters: set, dialogue_lines: List[str], action_lines: List[str]):
    """Categorize line as dialogue or action"""
...
def _aggregate_data(scenes: List[SceneData], script_content: str) -> RawScriptData:
    ...
    language = _detect_language(script_content)
...
def _detect_language(script_content: str) -> str:
...
def _fallback_extraction(script_content: str) -> RawScriptData:
    ...
    for line in lines:
        ...
        elif _is_character_name(line):
            clean_name = re.sub(r'\s*\([^)]*\)', '', line).strip()
            if clean_name:
                characters.add(clean_name)
    
    scenes = _create_fallback_scenes(scene_headers, characters, script_content)
...
def _create_fallback_scenes(scene_headers: List[str], characters: set, script_content: str) -> List[SceneData]:
    ...
    if scene_headers:
        ...
        for i, header in enumerate(scene_headers):
            ...
            scene = SceneData(
                ...
                dialogue_lines=[],
                action_lines=[],
                estimated_pages=max(1.0, len(script_content) / (WORDS_PER_PAGE * len(scene_headers))),
                props_mentioned=[],
                special_requirements=[]
            )
            scenes.append(scene)
        ...
        scenes = [SceneData(
            ...
            dialogue_lines=[],
            action_lines=[],
            estimated_pages=max(1.0, len(script_content) / WORDS_PER_PAGE),
            props_mentioned=[],
            special_requirements=[]
        )]
