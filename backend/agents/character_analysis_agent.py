...
FALLBACK_MESSAGE = "Unable to analyze"
...
async def analyze_characters(raw_data: RawScriptData) -> CharacterBreakdown:
    ...
    char_counts = _count_character_appearances(raw_data.scenes)
...
def _count_character_appearances(scenes: List[SceneData]) -> Dict[str, int]:
    """Count character appearances across scenes"""
    char_counts = {}
    for scene in scenes:
...
def _create_fallback_scene_character(scene: SceneData) -> SceneCharacterBreakdown:
    ...
    emotional_beats = ["Character development moment"] if scene.dialogue_lines else []
...
def _create_fallback_character_breakdown(scene_characters: List[SceneCharacterBreakdown], 
    ...
    main_threshold = total_scenes * MAIN_CHARACTER_THRESHOLD
    
    main_chars = [f"{char} - appears in {count} scenes" 
                  for char, count in char_counts.items() if count > main_threshold]
    supporting_chars = [f"{char} - appears in {count} scenes" 
                       for char, count in char_counts.items() if count <= main_threshold]
    casting_requirements = [f"Cast {char} - {count} scenes" 
