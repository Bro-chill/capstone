...
async def analyze_props(raw_data: RawScriptData) -> PropsBreakdown:
    ...
    try:
        all_props = _extract_all_props(scene_props)
...
def _extract_all_props(scene_props: List[ScenePropsBreakdown]) -> List[str]:
    """Extract all props from scene props"""
    all_props = []
...
def _create_fallback_scene_props(scene: SceneData) -> ScenePropsBreakdown:
    ...
    props_needed.extend(_get_location_props(scene.location))
...
def _get_location_props(location: str) -> List[str]:
    """Get props based on location type"""
    location_lower = location.lower()
    if "office" in location_lower:
        return ["desk", "chair", "computer"]
    elif "kitchen" in location_lower:
        return ["table", "chairs", "dishes"]
    return []
...
def _create_fallback_props_breakdown(scene_props: List[ScenePropsBreakdown], 
    ...
    categories = _categorize_props(master_props)
...
def _categorize_props(props: List[str]) -> Dict[str, List[str]]:
    """Categorize props by type"""
    furniture_props = [p for p in props if any(f in p.lower() for f in FURNITURE_KEYWORDS)]
    electronics_props = [p for p in props if any(e in p.lower() for e in ELECTRONICS_KEYWORDS)]
    other_props = [p for p in props if p not in furniture_props + electronics_props]
