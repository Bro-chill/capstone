...
def _create_fallback_scene_breakdown(scene: SceneData) -> DetailedSceneBreakdown:
    ...
    complexity = _determine_complexity(scene)
    action_ratio = _determine_action_ratio(scene)
...
def _determine_complexity(scene: SceneData) -> str:
...
def _determine_action_ratio(scene: SceneData) -> str:
...
def _create_fallback_scene_breakdown_overall(detailed_scenes: List[DetailedSceneBreakdown], 
    ...
    three_act = _create_three_act_structure(total_scenes)
...
def _create_three_act_structure(total_scenes: int) -> List[str]:
    """Create three-act structure breakdown"""
    act1_end = int(total_scenes * ACT_1_RATIO)
    act2_end = int(total_scenes * ACT_2_RATIO)
